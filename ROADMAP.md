...
**Follow-up (voice assistants):** Alexa voice commands ("Alexa, Licht 20%") send On and the brightness as two separate directives through the cloud pipeline — a measured 386 ms gap outlasted the original 250 ms defer window, so the deferred `LEVEL 1.005` still fired (the app sends a single brightness command, which is why only voice flashed). The window is now 500 ms, and a bare On is ignored entirely when the light is already on (voice assistants send it with every brightness directive regardless of state). If flashes reappear with slower voice pipelines, `dimmerOnDeferMs` is the knob — consider making it a config option before raising the default.
...
The DIMMER wiring in `module.ts` used two independent `subscribeAttribute` callbacks (`LevelControl.currentLevel` and `OnOff.onOff`). Both fired for a single user intent, producing **two CCU radio writes**:
