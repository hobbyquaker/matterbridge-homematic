...
export class TemplatePlatform extends MatterbridgeDynamicPlatform {
  ...
  private async applyInitialValuesFromRega(): Promise<void> {
    // fetchInitialValues returns an empty array when ReGa is not enabled or not connected,
    // so it is safe to call unconditionally — no network I/O occurs when ReGa is absent.
    // (Previously guarded by initialValuesFromRega config flag; now always runs.)
    if (!this.ccuConnection) return;
